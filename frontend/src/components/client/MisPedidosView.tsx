...
export function MisPedidosView({
  ...
  const { pedidos, setPedidos, clientes, productos, currentUser } = useStore();
  ...
  const handleCancelPedido = async () => {
    ...
    try {
      ...
      const myId = clientes.find((c) => c.email === currentUser?.email)?.id || currentUser?.id;
      const mapped = (response.data || [])
        .filter((o: any) => o.id_usuario_cliente?.toString() === myId?.toString())
        .map((o: any) => ({
          id: o.id_pedido.toString(),
          clienteId: o.id_usuario_cliente?.toString() || "",
          fecha: o.fecha_pedido?.split("T")[0] || "",
          productos: [],
          subtotal: 0, iva: 0, costoEnvio: 0,
  ...
  const handleSaveDireccion = async () => {
    if (!selectedPedido || !editDireccion.trim()) { toast.error("La dirección no puede estar vacía"); return; }
    setIsSavingDir(true);
    try {
      await orderService.updateDireccion(Number(selectedPedido.id), editDireccion.trim());
      toast.success("Dirección actualizada correctamente");
      setShowEditDir(false);
      // Actualizar el pedido seleccionado localmente
      setSelectedPedido({ ...selectedPedido, direccionEnvio: editDireccion.trim(), direccion: editDireccion.trim() });
    } catch (error: any) {
    ...
    <div style={{ minHeight: '100vh', background: '#fdf5f8' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 0' }}>
            ...
            {searchQuery.trim() ? (
              <button
                onClick={() => setSearchQuery("")}
                style={{
                  padding: '12px 28px', borderRadius: '10px', background: '#f9fafb',
                  color: '#374151', fontSize: '14px', fontWeight: 600, border: '1px solid #e5e7eb', cursor: 'pointer',
                }}
              >
                Limpiar búsqueda
              </button>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '20px' }}>
                  ...
                  {pedido.estado === 'pendiente' && (
                    <button
                      onClick={() => { setPedidoToCancel(pedido); setShowCancelConfirm(true); }}
                      style={{ padding: '8px 16px', borderRadius: '20px', background: '#fff', border: '1px solid #d1d5db', color: '#374151', fontSize: '12px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '6px' }}
          ...
          {selectedPedido && (
            <div style={{ display: "flex", height: "100%", width: "100%" }}>
              ...
              <div style={{
                ...
                background: "linear-gradient(160deg, #fdf8fa 0%, #ffffff 100%)",
              }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "18px", flex: 1 }}>
                  ...
                  {/* Order ID */}
                  <div>
                    <p style={{ fontSize: "9px", fontWeight: 900, letterSpacing: "4px", textTransform: "uppercase", color: "#c47b96", opacity: 0.8, marginBottom: "4px" }}>
                      Pedido
                    </p>
                    <h2 style={{ fontSize: "26px", fontWeight: 900, color: "#1f2937", letterSpacing: "-0.5px", margin: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                    ...
                    {selectedPedido.pago_confirmado && (
                      <div style={{
                        ...
                        <span style={{ fontSize: "9px", fontWeight: 900, color: "#16a34a", letterSpacing: "2px", textTransform: "uppercase" }}>
                          Pago OK
                        </span>
                      </div>
                    )}
                    ...
                    {selectedPedido.estado === "enviado" || selectedPedido.estado === "entregado" ? (
                      selectedPedido.numero_guia ? (
                        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                            <Truck style={{ width: 14, height: 14, color: "#c47b96" }} />
                            <span style={{ fontSize: "9px", fontWeight: 900, letterSpacing: "3px", textTransform: "uppercase", color: "#c47b96" }}>Datos de Envío</span>
                          </div>
                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                            <div>
                              <p style={{ fontSize: "9px", fontWeight: 900, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "4px" }}>Empresa</p>
                              <p style={{ fontSize: "13px", fontWeight: 700, color: "#1f2937", margin: 0 }}>{selectedPedido.transportadora}</p>
                            </div>
                            <div>
                              <p style={{ fontSize: "9px", fontWeight: 900, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "4px" }}>N. Guía</p>
                              <p style={{ fontSize: "13px", fontWeight: 700, color: "#c47b96", fontFamily: "monospace", margin: 0 }}>{selectedPedido.numero_guia}</p>
                            </div>
                          </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                          <Truck style={{ width: 18, height: 18, color: "#c47b96", flexShrink: 0 }} />
                          <p style={{ fontSize: "11px", color: "#6b7280", margin: 0, lineHeight: 1.5 }}>
                            Tu pedido está en camino. Los datos de guía estarán disponibles pronto.
                          </p>
                        </div>
                      )
                    ) : (
                      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        <Clock style={{ width: 18, height: 18, color: "#d1d5db", flexShrink: 0 }} />
                        <p style={{ fontSize: "11px", color: "#9ca3af", fontStyle: "italic", margin: 0, lineHeight: 1.5 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "12px" }}>
                    <MapPin style={{ width: 14, height: 14, color: "#c47b96", marginTop: "3px", flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: "9px", fontWeight: 900, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "4px" }}>Entrega en</p>
                      ...
                      {['pendiente', 'preparado'].includes(selectedPedido.estado) && (
                        <button
                          onClick={() => { setEditDireccion(selectedPedido.direccion || selectedPedido.direccionEnvio || ""); setShowEditDir(true); }}
                          style={{ marginTop: "8px", fontSize: "10px", fontWeight: 700, color: "#c47b96", background: "none", border: "none", cursor: "pointer", padding: 0, textDecoration: "underline" }}
                        >
                          Cambiar dirección
                        </button>
                      )}
                  ...
                  {/* Date */}
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "12px" }}>
                    <Calendar style={{ width: 14, height: 14, color: "#c47b96", marginTop: "3px", flexShrink: 0 }} />
                    <div>
                      <p style={{ fontSize: "9px", fontWeight: 900, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "4px" }}>Fecha</p>
                      <p style={{ fontSize: "12px", color: "#374151", margin: 0 }}>{formatDate(selectedPedido.fecha)}</p>
                    </div>
                  </div>
                </div>

                {/* Total at bottom */}
                <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: "16px", marginTop: "18px" }}>
                  <p style={{ fontSize: "9px", fontWeight: 900, color: "#c47b96", textTransform: "uppercase", letterSpacing: "4px", marginBottom: "4px" }}>Total</p>
                  <p style={{ fontSize: "28px", fontWeight: 900, color: "#1f2937", letterSpacing: "-1px", margin: 0 }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "28px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <Package style={{ width: 16, height: 16, color: "#c47b96" }} />
                    <span style={{ fontSize: "10px", fontWeight: 900, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "4px" }}>
                      Artículos ({selectedPedido.productos.length})
                    </span>
                  </div>
                  <div style={{ width: "40px", height: "2px", background: "#f0d5e0", borderRadius: "2px" }} />
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: "13px", fontWeight: 800, color: "#1f2937", margin: "0 0 4px 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <p style={{ fontSize: "16px", fontWeight: 900, color: "#1f2937", margin: 0 }}>
        ...
        <DialogContent className="bg-white border border-gray-100 max-w-md rounded-2xl shadow-2xl p-0 overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-6 pb-5 border-b border-gray-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="text-base font-bold text-gray-900 leading-tight">Cancelar Pedido</DialogTitle>
                <DialogDescription className="text-xs text-gray-400 mt-0.5">Esta acción no se puede deshacer</DialogDescription>
              </div>
            </div>
            <button onClick={() => setShowCancelConfirm(false)} className="p-1.5 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            <div style={{ background: "#fef2f2", borderRadius: "12px", padding: "16px", display: "flex", alignItems: "flex-start", gap: "12px", border: "1px solid #fecaca" }}>
              ...
              <div>
                <p style={{ fontSize: "14px", color: "#374151", lineHeight: 1.5 }}>
                  ¿Estás segura de cancelar el pedido <strong>#{pedidoToCancel?.id?.slice(0, 8).toUpperCase()}</strong>?
                </p>
          ...
          <div className="flex justify-end gap-3 px-6 pb-6 pt-2">
            <Button variant="outline" onClick={() => setShowCancelConfirm(false)} className="border-gray-200 text-gray-600 hover:bg-gray-50 rounded-lg px-5 h-10 text-sm" disabled={isCancelling}>Volver</Button>
            <Button onClick={handleCancelPedido} disabled={isCancelling} style={{ backgroundColor: "#ef4444", color: "#ffffff" }} className="rounded-lg font-semibold px-6 h-10 text-sm border-0">
              {isCancelling ? <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Cancelando...</div> : "Sí, cancelar"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showEditDir} onOpenChange={setShowEditDir}>
        <DialogContent className="bg-white border border-gray-100 max-w-md rounded-2xl shadow-2xl p-0 overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-6 pb-5 border-b border-gray-100">
            <div className="flex items-center gap-4">
              <div className="flex items-center justify-center flex-shrink-0" style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#c47b96,#e092b2)", boxShadow: "0 2px 8px rgba(196,123,150,0.3)" }}>
                <MapPin className="w-5 h-5 text-white" />
              </div>
              <div>
                <DialogTitle className="text-base font-bold text-gray-900 leading-tight">Cambiar Dirección</DialogTitle>
                <DialogDescription className="text-xs text-gray-400 mt-0.5">Pedido #{selectedPedido?.id?.slice(0, 8).toUpperCase()}</DialogDescription>
              </div>
            </div>
            <button onClick={() => setShowEditDir(false)} className="p-1.5 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div style={{ padding: "20px 24px" }}>
            <label className="text-sm font-semibold text-gray-700 block mb-2">Nueva dirección de envío</label>
            <input
              value={editDireccion}
          ...
          <div className="flex justify-end gap-3 px-6 pb-6 pt-2">
            <Button variant="outline" onClick={() => setShowEditDir(false)} className="border-gray-200 text-gray-600 hover:bg-gray-50 rounded-lg px-5 h-10 text-sm" disabled={isSavingDir}>Cancelar</Button>
            <Button onClick={handleSaveDireccion} disabled={isSavingDir} className="rounded-lg font-semibold px-6 h-10 text-sm border-0" style={{ backgroundColor: "#c47b96", color: "#ffffff" }}>
              {isSavingDir ? <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Guardando...</div> : "Guardar"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      {selectedPedido && (
        ...
        >
          ...
          <div style={{ height: "4px", background: "#2e1020", marginBottom: "32px", marginLeft: "-40px", marginRight: "-40px" }} />

          <div className="flex justify-between items-start mb-12">
            <div className="flex items-center gap-4">
              {/* Logo */}
              <img src="/logo.png" alt="Glamour ML" style={{ width: 52, height: 52, borderRadius: 10, objectFit: "cover" }} />
              <div>
                <h1 className="text-3xl font-bold mb-1" style={{ color: "#2e1020" }}>
                  GLAMOUR ML
                </h1>
                <p className="text-gray-500 text-sm">TIENDA DE BELLEZA & CUIDADO PERSONAL</p>
                <p className="text-gray-400 text-xs">Medellín, Colombia</p>
              </div>
            </div>
            <div className="text-right">
              <div className="px-6 py-4 rounded-xl" style={{ border: "2px solid #c47b96" }}>
                <p className="font-bold text-sm uppercase tracking-widest mb-1 text-center" style={{ color: "#c47b96" }}>
                  COMPROBANTE DE PEDIDO
                </p>
                <p className="font-bold text-2xl text-center" style={{ color: "#2e1020" }}>
                  #{selectedPedido.id.slice(0, 8).toUpperCase()}
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 gap-12 mb-12 py-8 border-t border-b border-gray-100">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Estado:{" "}
                <span className="font-bold uppercase" style={{ color: "#c47b96" }}>
          ...
          <table className="w-full mb-12">
            <thead>
              <tr className="text-xs font-bold uppercase tracking-widest" style={{ borderBottom: "2px solid #2e1020", color: "#2e1020" }}>
            ...
            <tbody className="divide-y divide-gray-100">
              {selectedPedido.productos.map((item: any, i: number) => {
                ...
                return (
                  <tr key={i} className="text-sm" style={{ background: i % 2 !== 0 ? "#f9fafb" : "transparent" }}>
                    <td className="py-4">
                    ...
                    <td className="py-4 text-center font-medium" style={{ color: "#2e1020" }}>
                      {item.cantidad}
                    </td>
                    <td className="py-4 text-right text-gray-600">
                      {formatCurrency(item.precioUnitario)}
                    </td>
                    <td className="py-4 text-right font-bold" style={{ color: "#2e1020" }}>
                      {formatCurrency(item.cantidad * item.precioUnitario)}
                    </td>
                  </tr>
                );
          ...
          <div className="flex justify-end pt-8" style={{ borderTop: "2px solid #2e1020" }}>
            <div className="w-80 space-y-3">
              ...
              <div className="flex justify-between text-2xl font-bold pt-3 border-t border-gray-100" style={{ color: "#2e1020" }}>
                <span>TOTAL:</span>
                <span>{formatCurrency(selectedPedido.total)}</span>
              </div>
            </div>
          ...
          <div className="mt-16 text-center bg-gray-50 p-6 rounded-xl">
            <p className="text-gray-400 text-xs italic mb-2">
              Comprobante digital. Su validez legal está sujeta a la normativa vigente.
            </p>

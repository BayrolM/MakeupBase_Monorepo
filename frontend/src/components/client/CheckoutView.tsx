...
export function CheckoutView({ onBack, onComplete }: CheckoutViewProps) {
  ...
  const [departamentoEnvio, setDepartamentoEnvio] = useState(
  ...
  const handleConfirmPayment = async () => {
    ...
    for (const item of carrito) {
      try {
        ...
        if (item.cantidad > freshProduct.stock_actual) {
          toast.error(
            `Solo quedan ${freshProduct.stock_actual} unidades de "${freshProduct.nombre}"`,
            {
    ...
    try {
      ...
      setGeneratedOrderId(
        response.id_pedido?.toString() || generateOrderNumber(),
      );
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-8">
        <div className="max-w-md w-full text-center">
          ...
          <div className="bg-surface border border-border rounded-lg p-4 mb-6">
            ...
            >
              Tu pedido está siendo procesado. Recibirás una confirmación por
              email.
            </p>
          </div>
                ...
                style={{
                  ...
                  background:
                    comprobanteFile && !isUploading
                      ...
                      : "#e5e7eb",
                  color:
                    comprobanteFile && !isUploading ? C.white : C.textMuted,
                  border: "none",
                  cursor:
